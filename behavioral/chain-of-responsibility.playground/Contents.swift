...
///
/// - Description: avoid copling the sender of a request to its receiver by giving more than one object a chance to handle the request. Chain the receiving objects and pass the request along the chain until an object handles it.
///
/// ## Advantages:
///     - reduced coupling.
...
///
/// ## Disadvantages:
///     - receipt isn't guaranteed. Since a request has no explicity receiver, there's no guarantee it'll be handled - the request can fall off the end of the chain without ever being handled. A request can also ho unhandled when the chain is not configured properly.
///
/// ## When to use it:
